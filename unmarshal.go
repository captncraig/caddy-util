...
func Unmarshal(c *setup.Controller, v interface{}) error {
	...
	for i, a := range args {
		field := findFieldForArg(&rv, i)
		if field == nil {
			return c.ArgErr()
		}
		if err := packField(field, []string{a}, c); err != nil {
			return err
		}
	}
	for c.NextBlock() {
...
func packField(field *reflect.Value, args []string, c *setup.Controller) error {
...
func findFieldForArg(rv *reflect.Value, i int) *reflect.Value {
	targetTag := fmt.Sprintf("arg%d", i)
	rt := rv.Type()
	for i := 0; i < rt.NumField(); i++ {
		f := rt.Field(i)
		tags := f.Tag.Get("caddy")
		tagParts := strings.Split(tags, ",")
		if len(tagParts) > 1 && tagParts[1] == targetTag {
